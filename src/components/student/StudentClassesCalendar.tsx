...
export function StudentClassesCalendar({
  classes,
  className,
}: {
  classes: StudentClassLike[];
  className?: string;
}) {
  ...
  const dayEvents = useMemo(() => {
    ...
    return monthGrid.map((day) => {
      ...
      for (const cls of activeClasses) {
        const scheduleData = cls.schedules;
        ...
        for (const s of scheduleData) {
          ...
          events.push({
            key: `${cls.id}-${s?.day_of_week}-${slot ?? ''}-${s?.room ?? ''}`,
      ...
      events.sort(
        (a, b) =>
          a.label.localeCompare(b.label) || String(a.time || '').localeCompare(String(b.time || ''))
      );

      // De-dupe repeated schedule entries
            ...
            return (
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  {hidden > 0 ? <div className="text-xs text-muted-foreground">+{hidden} more</div> : null}
                </div>
              </div>
            );
          })}
